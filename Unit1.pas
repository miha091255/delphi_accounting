...
begin
  ...
  try
    ...
    showMessage('ѕри попытке добавлени€ записи в базу данных произошла ошибка');
  end
  else
    showMessage('ƒл€ добавлени€ записи в базу данных заполните поле ‘»ќ');
end;
...
begin
  ...
  if not AttestatScan2.Checked then
    s := s + ' - нет копии приложени€ к аттестату;'+#13;
  if Family.Checked then
    s := s + ' - нет сведений о родител€х/ законных представител€х;'+#13;
  if Agreement.Checked then
    s := s + ' - нет за€влени€ от родителей о согласии на обработку ваших персональных данных;'+#13;
  if Grades.Checked then
    s := s + ' - не проставлены баллы за экзамены;'+#13;
  s := s + '≈сли у вас возникли вопросы по заполнению за€влени€,'+
  ...
  '≈сли у вас все еще остались вопросы, вы можете задать их '+questAddress;
  memo1.Text := s;
end;
...
begin
  FRegCount := Value;
  lRegCount.Caption := '«арегистрированно абитуриентов: '+IntToStr(FRegCount);
end;

procedure TForm1.SetTotalCount(const Value: Integer);
begin
  FTotalCount := Value;
  lCount.Caption := '«а врем€ сессии добавлено '+IntToStr(FTotalCount)+' записей';
end;
